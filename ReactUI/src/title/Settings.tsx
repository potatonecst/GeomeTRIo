...
type SettingItemDef = {
    id: string;
    label: string;
    type: 'slider' | 'toggle' | 'text' | 'button' | 'stat'; // slider: 数値, toggle: 真偽値, text: 文字列, button: 実行ボタン, stat: 統計情報(読取専用)
    description: string;
...
export const Settings = ({ onBack }: SettingsProps) => {
    ...
    useEffect(() => {
        ...
        (window as any).onMenuInput = (event: string) => {
            ...
            const playerName = values['player_name'] as string;
    ...
    const formatTime = (seconds: number) => {
        ...
        const m = Math.floor((seconds % 3600) / 60); // 残りの秒数を60で割って分を算出
        const s = Math.floor(seconds % 60); // 60で割った余りが秒
        ...
        <view className="flex-col w-full h-full p-12 text-white font-mono transition-opacity duration-300" style={{ opacity }}>
            ...
            <view className="flex-row w-full flex-1">
                ...
                <view className="w-3/4 pl-8 flex-col h-full">
                    ...
                    <view className={`flex-col flex-1 ${focusArea === 'item' ? 'opacity-100' : 'opacity-60'}`}>
