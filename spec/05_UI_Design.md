...
*   **オフライン表示 (OfflineIndicator)**:
    *   画面左下（コピーライトの上）に配置。
    *   クラウド接続に失敗した場合のみ "OFFLINE MODE" と警告アイコンを表示する。
    *   赤色で点滅し、異常状態であることを伝える。
*   **開始プロンプト**:
...
*   **Status Monitor**:
    ...
    *   **トースト通知 (Toaster)**:
        *   画面右上にシステムメッセージ（"DATA SAVED", "OFFLINE MODE" 等）を表示。
        *   C#側から制御され、メッセージがある間だけ表示される（常駐型）。
        *   オフライン時は "OFFLINE MODE" インジケーターも左上に表示される。
    *   **システムアラート (System Alert)**:
        *   データの競合や初期化など、重大な確認が必要な場合に表示されるモーダルダイアログ。
        *   **外観**: 画面全体を不透明度95%の黒で覆い、中央に赤枠のウィンドウを表示。
        *   **演出**: 背景のグリッチノイズと赤色の点滅により、危険な状態であることを強調する。
        *   **競合発生時 (Conflict)**:
            *   メッセージ: "DATA CONFLICT DETECTED. CLOUD DATA IS NEWER."
            *   選択肢:
                *   **OVERWRITE LOCAL**: クラウドのデータで上書きする（推奨）。
                *   **FORCE SAVE**: ローカルのデータでクラウドを強制上書きする（危険）。
*   **ゲームオーバー**:
