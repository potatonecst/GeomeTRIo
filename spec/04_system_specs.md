...
- **HUD表示:**
  ...
  - **トースト通知:** 画面右上に、セーブ状態（SAVING..., DATA SAVED）や接続状態（CONNECTING...）などのシステムフィードバックを一時的に表示する。
...
## 3. 技術・データ管理
- **セーブデータ管理:**
  - **保存形式:** クラウドストレージ (AWS DynamoDB)
  - **アーキテクチャ:** Unity -> AWS Lambda (Node.js) -> DynamoDB
  - **識別子:** `userId` (GUID) を `PlayerPrefs` に保存し、これをキーとしてクラウド上のデータを読み書きする。
  - **データ構造 (`GameData` クラス):**
  ...
  - **責任分界:**
    - `CloudSaveManager`: AWS Lambdaとの通信（HTTPリクエスト）を担当。
    - `GameManager`: ゲームロジック内での保存タイミングの制御と、オフラインモードの管理を担当。
  - **オフラインモード (Offline Mode):**
    - **発動条件:** 起動時のデータロード失敗、またはプレイ中のセーブ失敗時。
    - **挙動:**
      - 画面に "OFFLINE MODE" インジケーターを表示。
      - 以降の自動セーブ処理をスキップし、ローカルデータの不整合による上書きを防ぐ。
      - タイトル画面（Press Any Button）に戻ることで再接続を試行可能。
  - **データ整合性 (Data Consistency):**
    - **楽観的ロック (Optimistic Locking):**
      - サーバーからデータ取得時に最終更新日時 (`updatedAt`) を受け取り、メモリ上に保持する。
      - 保存時にこの日時を `prevUpdatedAt` として送信し、サーバー側でDB上の日時と比較する。
      - 日時が不一致（＝他の端末等で更新されている）の場合、サーバーは `409 Conflict` エラーを返す。
    - **競合解決 (Conflict Resolution):**
      - 競合エラー発生時は `SystemAlert` を表示し、ユーザーに「クラウドデータで上書き (Reload)」か「強制保存 (Force Save)」かを選択させる。
  - **データ保持ポリシー (TTL):**
    - DynamoDBのTTL機能を使用し、最終更新から **1年経過後** に自動削除される。
    - これにより、不要なゲストデータの蓄積を防ぐ。
...
- **コーディング規約:**
  ...
  - 非同期処理: コルーチン (`IEnumerator`) を主に使用（WebGLビルド等への互換性重視のため）。
