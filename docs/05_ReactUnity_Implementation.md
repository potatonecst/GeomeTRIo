...
*   **HUD (Head-Up Display)**:
    ...
    *   **オフラインインジケーター:**
        *   `isOffline` フラグが `true` の場合、HUD内（画面左上など）に "OFFLINE" というテキストを赤色で点滅表示します。
        *   Status Monitorとは独立して表示され、クラウドセーブが機能していない状態であることをプレイヤーに明確に伝えます。
...
### 2.16 SystemAlert (`src/components/SystemAlert.tsx`)
セーブデータの競合（コンフリクト）など、システムレベルの重要な警告を表示するモーダルコンポーネントです。
*   **役割:**
    *   `window.onSaveConflict` イベントを監視し、競合発生時に自動的に表示されます。
    *   ユーザーに「クラウドデータで上書き (Reload)」か「ローカルデータで強制上書き (Force Save)」の二択を提示します。
*   **実装:**
    *   **最前面表示:** `z-index` を高く設定し、画面全体を覆うオーバーレイとして描画されます。
    *   **入力ブロック:** 表示中は他のUIへの入力を遮断し、解決するまでゲーム進行を停止させます。
    *   **API連携:** 選択結果に応じて `interop.ReloadSaveData()` または `interop.ForceSaveData()` を呼び出し、その後ダイアログを閉じます。

### 2.17 Toaster (`src/game/Toaster.tsx`)
画面右上にシステムメッセージを一時的に表示するコンポーネントです。

*   **役割:**
    *   セーブ状態（SAVING..., DATA SAVED）や接続状態（CONNECTING...）など、バックグラウンドで進行する処理の状況をユーザーにフィードバックします。
*   **データソース:**
    *   `useGameStatus` フックから `toastMessage`, `toastId`, `toastTime` を取得します。
*   **実装:**
    *   **表示トリガー:** `toastId` が変化したことを検知して、新しいメッセージとして表示を開始します。これにより、同じメッセージが連続で送られてきても再表示（チラつき）されるのを防ぎます。
    *   **自動消去:** `useEffect` 内で `setTimeout` を使用し、一定時間（例: 3秒）が経過すると自動的にフェードアウトします。
    *   **永続表示:** `duration` が `0` のメッセージ（例: "SAVING..."）は、新しいメッセージで上書きされるまで表示され続けます。

### 2.18 KeyIcon (`src/components/KeyIcon.tsx`)
