...
- **CloudSaveManager:** AWS Lambda + DynamoDB との通信を担当するシングルトンクラス。
...
*   **記録項目:** 総プレイ時間、総撃破数、総プレイ回数、総被ダメージ数、総与ダメージ数、総発射数、累計スコア、SP使用回数、誘爆撃破数、アイテム取得数。
*   **更新タイミング:**
    *   敵撃破時、被弾時、発射時（メモリ上での加算）。
    *   ゲームオーバー時、タイトルへ戻る時（クラウドへの保存）。
    *   ※ゲーム開始時のプレイ回数加算はメモリ上で行い、保存は終了時に一括で行うことで、演出中の通信遅延やエラーを防ぐ。
...
*   **セーブデータ (`GameData`):** ユーザーの進行状況やプレイスタイルに関わる設定（HP, SP, AutoFire, PlayerName）。`GameManager` が保持し、`CloudSaveManager` を通じてクラウド（DynamoDB）に保存されます。
    *   **排他制御:** 複数端末での利用や通信ラグによる先祖返りを防ぐため、`updatedAt` を用いた楽観的ロックを実装しています。競合発生時はUIを通じてユーザーに解決を求めます。
*   **グローバル設定 (`PlayerPrefs`):** アプリケーション全体で共有される環境設定（BGM/SE音量, 振動）。`SettingsManager` が管理。
    *   **認証トークン:** クラウドセーブの本人確認用トークンも `PlayerPrefs` に保存されます。
...
### 7.3 タイトルへ戻るシーケンス
ゲームプレイからタイトル画面に戻る際、スムーズな遷移と演出のスキップを行います。

1.  **中断/終了:** ユーザーがポーズメニューやリザルト画面から「タイトルへ」を選択。
2.  **ローディング表示:** React側でローディング画面を表示。
3.  **非同期ロード:** `GameManager` が `TitleScene` を非同期で読み込み開始。
4.  **フラグ設定:** `GameManager.SkipTitleSequence = true` に設定。
5.  **暗転:** ロード完了後、暗転。
6.  **シーン遷移:** タイトルシーンへ切り替え。
7.  **演出スキップ:** `TitleApp` (React) がマウント時に `interop.ShouldSkipTitleSequence()` を確認。
    *   `true` が返ってきた場合、"Press Any Button" や接続演出をスキップし、即座にメインメニュー (`connected` 状態) を表示します。
