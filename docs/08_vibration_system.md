...
## 実装済み振動パレット (Vibration Palette)

本プロジェクトで使用している振動パラメータの一覧です。
`Vibrate(low, high, duration, priority)`

| アクション         | Low (重) | High (鋭) | Time (秒) | Priority | 解説                                                |
| :----------------- | :------- | :-------- | :-------- | :------- | :-------------------------------------------------- |
| **カーソル移動**   | 0.0      | 0.1       | 0.03      | 0.5      | ごく微弱なクリック感。                              |
| **決定 (Submit)**  | 0.0      | 0.3       | 0.05      | 1.0      | 鋭い「カチッ」という感触。                          |
| **キャンセル**     | 0.3      | 0.0       | 0.05      | 1.0      | 鈍い「コツッ」という感触。                          |
| **射撃 (Fire)**    | 0.0      | 0.1       | 0.05      | 0.5      | 非常に弱い反動。連射しても不快にならない程度。      |
| **被弾 (Damage)**  | 0.8      | 0.5       | 0.4       | 2.0      | **重い衝撃。** 優先度が高く、射撃振動を上書きする。 |
| **スピンアタック** | 0.4      | 0.6       | 2.0       | 1.5      | 回転の重みと攻撃の激しさを表現。効果時間中続く。    |
